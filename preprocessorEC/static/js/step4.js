document.addEventListener('DOMContentLoaded', function() {
    ...
    if (startInforCLMatchingBtn) {
        startInforCLMatchingBtn.addEventListener('click', function() {
            ...
            fetch(getApiUrl('/item-matching/match-infor-contract-lines'), {
                method: 'POST'
            })
                ...
                if (data.success) {
                    try {
                        ...
                        if (data.result) {
                            ...
                            // smoothly scroll to the top of the comparison container
    ...
    selectAllBtn.addEventListener('click', function() {
        document.querySelectorAll('#items-tbody-4-1 tr:not([style*="display: none"]) input[type="checkbox"]').forEach(checkbox => {
    ...
    function renderItemsTable(items) {
        ...
        try {
            ...
            items.forEach((item, index) => {
                ...
                const matchClass = isExactMatch ? 'match' : 'no-match';
    ...
    function saveSelections() {
        ...
        fetch(getApiUrl('/item-matching/update-infor-cl-false-positives'), {
            method: 'POST',
            headers: {
    ...
    if (completeStep41Btn) {
        completeStep41Btn.addEventListener('click', function() {
            ...
            fetch(getApiUrl('/item-matching/match-item-master'), {
                method: 'POST'
            })
            .then(response => response.json())
            .then(data => {
                ...
                if (data.success) {
                    try {
                        ...
                        // smoothly scroll to the itemmaster table container
                        itemMasterTableContainer.scrollIntoView({ behavior: 'smooth' });
                    } catch (err) {
    ...
    function processItemMasterResults(results) {      
        ...
        safeSetDisplay(itemMasterTableContainer.querySelector('.items-table-container'), 'none');
        // Still show the results container for stats
        ...
        try {
            ...
            if (sortField42) {
                items = [...items].sort((a, b) => {
                    ...
                    // special handling for vendor_id_match
                    if (sortField42 === 'vendor_id_match') {
                        valA = a.vendor_id_match === true ? 1 : 0;
                        valB = b.vendor_id_match === true ? 1 : 0;
                    }
            ...
            items.forEach((item, index) => {
                ...
                // Is exact match with safeguard for MFN
                const isExactMatch = item.same_mfg_part_num === 1;
                const matchClass = isExactMatch ? 'match' : 'no-match';
                
                // Check for exact vendor ID match
                const inforVendorId = item.erp_vendor_id_infor || '';
                const uploadVendorId = item.ERP_Vendor_ID || '';
                const isVendorIdMatch = inforVendorId && uploadVendorId && 
                                    inforVendorId.toString() === uploadVendorId.toString();
                const vendorMatchClass = isVendorIdMatch ? 'match' : 'no-match';
                
                item.vendor_id_match = isVendorIdMatch;
                ...
                row.innerHTML = `
                    ...
                    <td><span class="match-indicator ${vendorMatchClass}"></span></td>
                    <td>${inforVendorId}</td>
    ...
    function attachSort42Listeners() {
    ...
    function saveFalsePositiveSelections() {
        ...
        // Send updates to the backend
        fetch(getApiUrl('/item-matching/update-item-master-false-positives'), {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
    ...
    if (selectAllBtn42) {
        selectAllBtn42.addEventListener('click', function() {
            document.querySelectorAll('#items-tbody-4-2 tr:not([style*="display: none"]) input.false-positive-checkbox').forEach(checkbox => {
                checkbox.checked = true;
            });
        });
    }
    ...
    clearSearchBtn42.addEventListener('click', function() {
        itemSearchInput42.value = '';
        itemSearchType42.value = 'contains';
        filterItemMasterTable();
    });
    ...
    // handle the transition from step 4.2 to 4.3
    const completeStep42Btn = document.getElementById('complete-step-4-2-btn');
    const step43Content = document.getElementById('step-4-3-content');
    const startUomQoeValidationBtn = document.getElementById('start-uom-qoe-validation-btn');

    if (completeStep42Btn) {
        completeStep42Btn.addEventListener('click', function() {
            // Show loading overlay
            safeSetDisplay(loadingOverlay, 'flex');
            
            // Show Step 4.3
            safeSetDisplay(step43Content, 'block');
            
            // Hide the "Start UOM/QOE Validation" button since we're automatically starting it
            safeSetDisplay(startUomQoeValidationBtn, 'none');
            
            // Automatically call the UOM/QOE validation API
            fetch(getApiUrl('/item-matching/validate-uom-qoe'), {
                method: 'POST'
            })
            .then(response => response.json())
            .then(data => {
                ...
                if (data.success) {
                    try {
                        // Process UOM/QOE validation results
                        processUomQoeResults(data.result);
                        
                        // Show results container and action buttons
                        safeSetDisplay(document.getElementById('uom-qoe-results-container'), 'block');
                        safeSetDisplay(document.getElementById('uom-qoe-table-container'), 'block');
                        safeSetDisplay(document.getElementById('action-buttons-4-3'), 'block');
                         // smoothly scroll to the uom-qoe table container
                        document.getElementById('uom-qoe-table-container').scrollIntoView({ behavior: 'smooth' });
                    } catch (err) {
                        showError("Error processing UOM/QOE validation results: " + err.message);
                        console.error("Processing error details:", err);
                    }
                } else {
                    showError("Error validating UOM/QOE: " + data.message);
                }
            })
            .catch(error => {
                safeSetDisplay(loadingOverlay, 'none');
                showError("Error during UOM/QOE validation: " + error.toString());
                console.error("Full error details:", error);
            });
        });
    }

    if (startUomQoeValidationBtn) {
        startUomQoeValidationBtn.addEventListener('click', function() {
            // Show loading overlay
            safeSetDisplay(loadingOverlay, 'flex');
    ...
    // Variables for UOM/QOE table sorting
    let allUomQoeItems = [];
    let sortFieldUomQoe = null;
    let sortDirectionUomQoe = 'asc';

    // function to process the UOM/QOE validation results
    function processUomQoeResults(result) {
        if (!result || !result.analyzed_df || result.analyzed_df.length === 0) {
            safeSetDisplay(document.getElementById('no-discrepancies-message'), 'block');
            safeSetDisplay(document.querySelector('#uom-qoe-table-container .items-table-container'), 'none');
            return;
        }
        
        allUomQoeItems = result.analyzed_df || [];
        
        // Update stats
        document.getElementById('total-items-validated').textContent = result.total_validation_count || 0;
        document.getElementById('uom-discrepancies').textContent = result.failed_count || 0;
        document.getElementById('false-positive-count').textContent = result.false_positive_count || 0;
        
        // Set the "All Pass" status
        const allPassElement = document.getElementById('all-pass-status');
        if (allPassElement) {
            const allPassValue = result.all_pass_flag === true ? "All Passed" : "With Issues";
            allPassElement.textContent = allPassValue;
            
            // Change text color based on status
            allPassElement.style.color = result.all_pass_flag === true ? "#2ecc71" : "#e74c3c";
            
            // Also change the card's border color based on status
            const allPassCard = document.querySelector('.summary-stats-row.four-columns .summary-stats-column:first-child .summary-stat-card');
            if (allPassCard) {
                if (result.all_pass_flag === true) {
                    allPassCard.className = 'summary-stat-card success-card';  // Green border
                } else {
                    allPassCard.className = 'summary-stat-card false-positive-card';  // Red border
                }
            }
        }
        
        // Always render the table with all items
        renderUomQoeTable(allUomQoeItems);
        
        // Always show the table container if there are items
        if (allUomQoeItems.length > 0) {
            safeSetDisplay(document.getElementById('no-discrepancies-message'), 'none');
            safeSetDisplay(document.querySelector('#uom-qoe-table-container .items-table-container'), 'block');
            
            // Attach sort listeners
            attachUomQoeSortListeners();
        } else {
            safeSetDisplay(document.getElementById('no-discrepancies-message'), 'block');
            safeSetDisplay(document.querySelector('#uom-qoe-table-container .items-table-container'), 'none');
        }
    ...
    // Function to attach sort listeners
    function attachUomQoeSortListeners() {
        document.querySelectorAll('#uom-qoe-table th[data-sort]').forEach(th => {
            ...
            newTh.addEventListener('click', function() {
                ...
                // Update sort direction
                if (field === sortFieldUomQoe) {
                    sortDirectionUomQoe = sortDirectionUomQoe === 'asc' ? 'desc' : 'asc';
                } else {
                    sortFieldUomQoe = field;
                    sortDirectionUomQoe = 'asc';
                }
                
                // Update sort indicators
                document.querySelectorAll('#uom-qoe-table th').forEach(header => {
                    header.classList.remove('sort-asc', 'sort-desc');
                });
                this.classList.add(sortDirectionUomQoe === 'asc' ? 'sort-asc' : 'sort-desc');
    ...
    // function to render the UOM/QOE discrepancies table
    function renderUomQoeTable(items) {
        const uomQoeTbody = document.getElementById('uom-qoe-tbody');
        uomQoeTbody.innerHTML = '';
        
        try {
            // Keep ALL items, regardless of validation status
            if (items.length === 0) {
                // If no items at all, show the no discrepancies message
                safeSetDisplay(document.getElementById('no-discrepancies-message'), 'block');
                safeSetDisplay(document.querySelector('#uom-qoe-table-container .items-table-container'), 'none');
                return;
            }
            ...
            if (sortFieldUomQoe) {
                items = [...items].sort((a, b) => {
                    let valA = a[sortFieldUomQoe];
                    let valB = b[sortFieldUomQoe];
                    ...
                    if (!isNaN(parseFloat(valA)) && !isNaN(parseFloat(valB))) {
                        return sortDirectionUomQoe === 'asc' ? 
                    ...
                    if (valA < valB) return sortDirectionUomQoe === 'asc' ? -1 : 1;
                    if (valA > valB) return sortDirectionUomQoe === 'asc' ? 1 : -1;
                    return 0;
                });
            }

            // Show all items with all requested fields
            items.forEach((item, index) => {
                const row = document.createElement('tr');
                
                // Format valid options as a list
                let validOptions = item["All Valid UOM*QOE"] || 'N/A';
                
                // Add a visual indicator for passed status
                const validationClass = item.Validation === "Passed" ? "bg-success-light" : "bg-danger-light";
                const validationStatus = item.Validation === "Passed" ? 
                    `<div class='${validationClass}'>✓ Valid</div>` : 
                    `<div class='${validationClass}'>✗ Invalid</div>`;
                
                // Handle the false positive value
                const isFalsePositive = item.False_Positive === true || item["False Positive"] === true;
                ...
                row.innerHTML = `
                    ...
                    <td>${item["File Row"] || 'N/A'}</td>
                    <td class="description-col" title="${item.ItemDescription || ''}">${(item.ItemDescription || 'N/A').substring(0, 50)}${(item.ItemDescription || '').length > 50 ? '...' : ''}</td>
                    <td class="description-col" title="${item.Description || ''}">${(item.Description || 'N/A').substring(0, 50)}${(item.Description || '').length > 50 ? '...' : ''}</td>
                    <td>${item["Mfg Part Num"] || 'N/A'}</td>
                    <td>${item["Vendor Part Num"] || 'N/A'}</td>
                    <td>${item["ERP Vendor ID"] || 'N/A'}</td>
                    <td>${item["Contract Number"] || 'N/A'}</td>
                `;
                
                uomQoeTbody.appendChild(row);
            });
            
            // Add event listeners to false positive checkboxes
            document.querySelectorAll('#uom-qoe-tbody input.false-positive-checkbox').forEach(checkbox => {
                checkbox.addEventListener('change', function() {
                    console.log(`UOM/QOE checkbox toggled: item ${checkbox.dataset.item}, file row ${checkbox.dataset.fileRow}, is now ${checkbox.checked ? 'checked' : 'unchecked'}`);
                });
            });
            
            // Always display the table container when there are items
            safeSetDisplay(document.getElementById('no-discrepancies-message'), 'none');
            safeSetDisplay(document.querySelector('#uom-qoe-table-container .items-table-container'), 'block');
            
        } catch (err) {
            console.error("Error rendering UOM/QOE table:", err);
            showError("Error rendering UOM/QOE results: " + err.message);
        }
    }



    // Get relevant elements for UOM/QOE controls
    const selectAllUomQoeBtn = document.getElementById('select-all-uom-qoe-btn');
    const deselectAllUomQoeBtn = document.getElementById('deselect-all-uom-qoe-btn');
    const saveUomQoeSelectionsBtn = document.getElementById('save-uom-qoe-selections-btn');
    const uomQoeSearch = document.getElementById('uom-qoe-search');
    const uomQoeSearchType = document.getElementById('uom-qoe-search-type');
    const clearUomQoeSearchBtn = document.getElementById('clear-uom-qoe-search-btn');

    // Add event listeners for UOM/QOE controls
    if (selectAllUomQoeBtn) {
        selectAllUomQoeBtn.addEventListener('click', function() {
            document.querySelectorAll('#uom-qoe-tbody tr:not([style*="display: none"]) input.false-positive-checkbox:not(:checked)').forEach(checkbox => {
                checkbox.checked = true;
                console.log(`Checked checkbox for item ${checkbox.dataset.item}, file row ${checkbox.dataset.fileRow}`);
            });
        });
    }

    if (deselectAllUomQoeBtn) {
        deselectAllUomQoeBtn.addEventListener('click', function() {
            document.querySelectorAll('#uom-qoe-tbody tr:not([style*="display: none"]) input.false-positive-checkbox:checked').forEach(checkbox => {
                checkbox.checked = false;
                console.log(`Unchecked checkbox for item ${checkbox.dataset.item}, file row ${checkbox.dataset.fileRow}`);
            });
        });
    }

    // Function to save UOM/QOE false positive selections
    function saveUomQoeFalsePositiveSelections() {
        // Show loading overlay
        safeSetDisplay(loadingOverlay, 'flex');
        
        // Collect all checked false positive items
        const falsePositiveItems = [];
        document.querySelectorAll('#uom-qoe-tbody input.false-positive-checkbox').forEach(checkbox => {
            falsePositiveItems.push({
                file_row: checkbox.dataset.fileRow || '',
                item: checkbox.dataset.item || '',
                is_false_positive: checkbox.checked
            });
        });
        
        console.log(`Saving ${falsePositiveItems.length} UOM/QOE false positive updates`);
        
        // Send updates to the backend
        fetch(getApiUrl('/item-matching/update-uom-qoe-false-positives'), {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
        ...
        .then(data => {
            ...
            if (data.success) {
                // Use alert() instead of showSuccess to match the other steps
                alert('Selections saved successfully');
                
                // Update ALL metric cards with the new values
                if (data.false_positive_count !== undefined) {
                    document.getElementById('false-positive-count').textContent = data.false_positive_count;
                }
                if (data.total_validation_count !== undefined) {
                    document.getElementById('total-items-validated').textContent = data.total_validation_count;
                }
                if (data.failed_count !== undefined) {
                    document.getElementById('uom-discrepancies').textContent = data.failed_count;
                }
                if (data.all_pass_flag !== undefined) {
                    const allPassElement = document.getElementById('all-pass-status');
                    if (allPassElement) {
                        const allPassValue = data.all_pass_flag === true ? "All Passed" : "No";
                        allPassElement.textContent = allPassValue;
                        allPassElement.style.color = data.all_pass_flag === true ? "#2ecc71" : "#e74c3c";
                    }
                }
            } else {
                alert(`Error saving selections: ${data.message}`);
    ...
    // Attach event listener to save selections button
    if (saveUomQoeSelectionsBtn) {
        saveUomQoeSelectionsBtn.addEventListener('click', function(e) {
            console.log("Save UOM/QOE selections clicked");
            e.preventDefault();
            saveUomQoeFalsePositiveSelections();
        });
    }

    // Add search functionality
    if (uomQoeSearch) {
        uomQoeSearch.addEventListener('input', filterUomQoeTable);
    }

    if (uomQoeSearchType) {
        uomQoeSearchType.addEventListener('change', filterUomQoeTable);
    }

    if (clearUomQoeSearchBtn) {
        clearUomQoeSearchBtn.addEventListener('click', function() {
            uomQoeSearch.value = '';
            filterUomQoeTable();
        });
    }

    // Function to filter UOM/QOE table
    function filterUomQoeTable() {
        const searchTerm = uomQoeSearch.value.toLowerCase();
        const searchType = uomQoeSearchType.value;
        
        document.querySelectorAll('#uom-qoe-tbody tr').forEach(row => {
            if (!searchTerm) {
                row.style.display = '';
                return;
            }
            ...
            if (searchType === 'all') {
                ...
                // Get the index of the column we want to search in
                const headers = Array.from(document.querySelectorAll('#uom-qoe-table th'));
                const columnIndex = headers.findIndex(th => th.dataset.sort === searchType);
    ...
    const generateValidationResultFileBtn = document.getElementById('generate-validation-result-file-btn');
    if (generateValidationResultFileBtn) {
        generateValidationResultFileBtn.addEventListener('click', function() {
            // Use a more robust approach for file downloads
            safeSetDisplay(loadingOverlay, 'flex'); // Show loading indicator
            
            // Create a form and submit it (more reliable than window.location.href)
            const form = document.createElement('form');
            form.method = 'GET';
            form.action = '/item-matching/download-uom-qoe-discrepancies'; // Fixed path with prefix
            document.body.appendChild(form);
            
            form.submit();
            
            // Hide loading overlay after a delay to give time for download to start
            setTimeout(() => {
                safeSetDisplay(loadingOverlay, 'none');
            }, 1000);
        });
    ...
    // handle the completion of step 4.3 and navigation to step 5
    const completeStep43Form = document.getElementById('complete-step-4-3-form');

    if (completeStep43Form) {
        completeStep43Form.addEventListener('submit', function(e) {
            e.preventDefault(); // Prevent default form submission
            
            // Make sure the form has the correct action attribute
            if (!completeStep43Form.getAttribute('action') || 
                completeStep43Form.getAttribute('action') !== '/process-step/4') {
                completeStep43Form.setAttribute('action', '/process-step/4');
            }
            
            // Show loading overlay
            safeSetDisplay(loadingOverlay, 'flex');
            
            // Submit the form using fetch API with the correct URL
            fetch(getApiUrl('/process-step/4'), {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/x-www-form-urlencoded',
                },
                body: new URLSearchParams(new FormData(completeStep43Form))
            })
            .then(response => {
                if (!response.ok) {
            ...
            .then(data => {
                ...
                if (data.success) {
                    ...
                    alert('Step 4 completed successfully!');
                    
                    // Redirect to step 5
                    window.location.href = '/step/5';
                } else {
                    alert('Error completing step 4: ' + (data.message || 'Unknown error'));
                }
            })
            .catch(error => {
                safeSetDisplay(loadingOverlay, 'none');
                console.error('Error:', error);
                alert('Error completing step 4: ' + error.message);
            });
        });
    }
});